...
If the Claude SDK network proxy is enabled and the effective `ANTHROPIC_BASE_URL` host is `localhost` or `127.0.0.1`, claude-mem adds only that host to `NO_PROXY` so the local LiteLLM gateway is reached directly. IPv6 loopback is not covered: use `localhost` or `127.0.0.1` instead of `[::1]` when the SDK proxy is enabled.
