...
### Claude SDK Network Proxy

Claude-Mem strips inherited proxy environment variables from child processes by default. To opt only the Claude Agent SDK subprocesses into an HTTP proxy, set both values in `~/.claude-mem/settings.json`:

```json
{
  "CLAUDE_MEM_CLAUDE_SDK_PROXY_ENABLED": "true",
  "CLAUDE_MEM_CLAUDE_SDK_PROXY_URL": "http://127.0.0.1:59625"
}
```

| Setting | Default | Description |
|---------|---------|-------------|
| `CLAUDE_MEM_CLAUDE_SDK_PROXY_ENABLED` | `false` | Inject the configured proxy into Claude Agent SDK subprocesses only |
| `CLAUDE_MEM_CLAUDE_SDK_PROXY_URL` | — | Exact `http://` or `https://` URL injected as both `HTTP_PROXY` and `HTTPS_PROXY` |

These two values are read only from `settings.json`; same-named parent environment variables and ordinary inherited proxy variables cannot override them. Enabling the proxy without a non-empty URL stops the SDK launch with an error. Local Chroma MCP, the worker MCP self-check, and the worker daemon keep their default proxy-stripping behavior.

When the effective `ANTHROPIC_BASE_URL` host is exactly `localhost` or `127.0.0.1`, claude-mem also injects that single host as `NO_PROXY` so the SDK reaches a local gateway directly. An unset or non-loopback base URL does not receive `NO_PROXY`. IPv6 loopback is not covered: an `[::1]` base URL leaves `NO_PROXY` unset and writes a warning to the worker log.
